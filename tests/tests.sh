...
# sssom parse $INPUT_FILE_1 --output $OUTPUT_DIR/parsed_basic.tsv --input-format tsv --curie-map-mode merged
# sssom parse $INPUT_URL_1 --output $OUTPUT_DIR/parsed_basic.tsv --input-format tsv --curie-map-mode merged
...
do
  sssom convert $INPUT_FILE_1 --output $OUTPUT_DIR/converted_basic.$fmt --output-format $fmt
  sssom convert $INPUT_URL_1 --output $OUTPUT_DIR/converted_basic.$fmt --output-format $fmt
done
...
# sssom dedupe $INPUT_FILE_1 --output $OUTPUT_DIR/deduped_basic.tsv
# sssom dedupe $INPUT_URL_1 --output $OUTPUT_DIR/deduped_basic.tsv

# sssom dosql -q "$QUERY_1" $INPUT_FILE_1 -o $OUTPUT_DIR/dosql_output.tsv
# sssom dosql -q "$QUERY_1" $INPUT_URL_1 -o $OUTPUT_DIR/dosql_output.tsv

# #sssom sparql 

# sssom diff $INPUT_FILE_1 $INPUT_FILE_2 -o $OUTPUT_DIR/diff_output.tsv
# sssom diff $INPUT_URL_1 $INPUT_URL_2 -o $OUTPUT_DIR/diff_output.tsv

# sssom partition -d $OUTPUT_DIR $INPUT_FILE_1 $INPUT_FILE_2 
# sssom partition -d $OUTPUT_DIR $INPUT_URL_1 $INPUT_URL_2

# sssom cliquesummary $INPUT_FILE_1 -o $OUTPUT_DIR/cliquesummary_output.tsv
# sssom cliquesummary $INPUT_URL_1 -o $OUTPUT_DIR/cliquesummary_output.tsv

# sssom crosstab $INPUT_FILE_1 -o $OUTPUT_DIR/crosstab_output.tsv
# sssom crosstab $INPUT_URL_1 -o $OUTPUT_DIR/crosstab_output.tsv

# sssom correlations $INPUT_FILE_1 -o $OUTPUT_DIR/correlation_output.tsv
# sssom correlations $INPUT_URL_1 -o $OUTPUT_DIR/correlation_output.tsv

# sssom merge $INPUT_FILE_1 $INPUT_FILE_2 $INPUT_FILE_3 -o $OUTPUT_DIR/merged_msdf.tsv
# sssom merge $INPUT_URL_1 $INPUT_URL_2 $INPUT_URL_3 -o $OUTPUT_DIR/merged_msdf.tsv