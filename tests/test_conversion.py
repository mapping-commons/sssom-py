...
from .test_data import ensure_test_dir_exists, SSSOMTestCase, TEST_CASES
...
class SSSOMReadWriteTestSuite(unittest.TestCase):
    ...
    def _test_graph_size(self, graph: Graph, queries: list, file: str):
        for query, size in queries:
            self.assertEqual(len(graph.query(query)), size, f"Graph query {query} does not return the expected number of triples for {file}")

    def _test_to_dataframe(self, mdoc, test):
        df = to_dataframe(mdoc)
        self.assertEqual(len(df), test.ct_data_frame_rows, f"The pandas data frame has less elements than the orginal one for {test.filename}")
        df.to_csv(test.get_out_file("roundtrip.tsv"), sep="\t")
        data = pd.read_csv(test.get_out_file("roundtrip.tsv"), sep="\t")
        self.assertEqual(len(data), test.ct_data_frame_rows, f"The re-serialised pandas data frame has less elements than the orginal one for {test.filename}")
        ...
        self.assertEqual(len(df), test.ct_data_frame_rows, f"The exported pandas data frame has less elements than the orginal one for {test.filename}")

    def _test_to_json_dict(self, mdoc: MappingSetDocument, test: SSSOMTestCase):
        json_dict = to_jsonld_dict(mdoc)
        self.assertEqual(len(json_dict), test.ct_json_elements, f"JSON document has less elements than the orginal one for {test.filename}")
        ...
        self.assertEqual(len(data), test.ct_json_elements, f"The re-serialised JSON file has less elements than the orginal one for {test.filename}")
        ...
        self.assertEqual(len(data), test.ct_json_elements, f"The exported JSON file has less elements than the orginal one for {test.filename}")
