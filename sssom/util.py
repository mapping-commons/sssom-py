...
def to_mapping_set_dataframe(doc: MappingSetDocument) -> MappingSetDataFrame:
    ...
    slots_with_double_as_range = [
        s
        for s in SSSOM_SCHEMA_OBJECT.dict["slots"].keys()
        if SSSOM_SCHEMA_OBJECT.dict["slots"][s]["range"] == "double"
    ]
    ...
    for property in map_obj:
        if map_obj[property] is not None:
            if isinstance(map_obj[property], list):
                ...
                if (
                    SSSOM_SCHEMA_OBJECT.dict["slots"][property]["range"]
                    in SSSOM_SCHEMA_OBJECT.dict["enums"].keys()
                ):
                    # IF object is a multivalued enum
                    if SSSOM_SCHEMA_OBJECT.dict["slots"][property]["multivalued"]:
...
def get_prefixes_used_in_table(df: pd.DataFrame) -> List[str]:
    ...
    if not df.empty:
        for col in SSSOM_SCHEMA_OBJECT.entity_reference_slots:
...
def is_multivalued_slot(slot: str) -> bool:
    ...
    return slot in SSSOM_SCHEMA_OBJECT.multivalued_slots
...
def reconcile_prefix_and_data(
    ...
    if len(data_switch_dict) > 0:
        # Read schema file
        slots = SSSOM_SCHEMA_OBJECT.dict["slots"]
...
def sort_df_rows_columns(
    ...
    if by_columns and len(df.columns) > 0:
        column_sequence = [
            col for col in SSSOM_SCHEMA_OBJECT.dict["slots"].keys() if col in df.columns
        ]
...
def get_all_prefixes(msdf: MappingSetDataFrame) -> list:
    ...
    if msdf.metadata and not msdf.df.empty:  # type: ignore
        ...
        ent_ref_slots = [
            s for s in all_keys if s in SSSOM_SCHEMA_OBJECT.entity_reference_slots
        ]
...
def augment_metadata(
    ...
    if msdf.metadata:
        for k, v in meta.items():
            ...
            if k in SSSOM_SCHEMA_OBJECT.multivalued_slots and not replace_multivalued:
                tmp_value: list = []
            ...
            elif k in SSSOM_SCHEMA_OBJECT.multivalued_slots and replace_multivalued:
...
def are_params_slots(params: dict) -> bool:
    ...
    legit_params = all(
        p in SSSOM_SCHEMA_OBJECT.mapping_set_slots for p in params.keys()
    )
    if not legit_params:
        invalids = [p for p in params if p not in SSSOM_SCHEMA_OBJECT.mapping_set_slots]
        raise ValueError(
            f"The params are invalid: {invalids}. Should be any of the following: {SSSOM_SCHEMA_OBJECT.mapping_set_slots}"
        )
    return True
