...
from sssom.sparql_util import expand_curie
from sssom.parsers import to_mapping_set_document
from sssom.util import MappingSetDataFrame

def rewire_graph(g: Graph, mset: MappingSetDataFrame, subject_to_object=True, precedence: List[str] = None) -> str:
    """
            ...
            if src in rewire_map:
                ...
                if precedence:
                    ...
                    if tgt_pfx in precedence:
                        if curr_pfx not in precedence or \
                                precedence.index(tgt_pfx) < precedence.index(curr_pfx):
                            rewire_map[src] = tgt
